...
namespace mtziLAB2
{
    ...
    {
        private string Kstring = "12345678";
        private BitArray[] K;
        private int[] IP_OTi;
        ...
        {
            BitArray temp = new BitArray(inputArr.Length);
            for (int i = 0; i < pos; i++)
            {
                temp[i + (inputArr.Length - pos)] = inputArr[i];
            }
        ...
        {
                //CheckInputData(shifrInputTextBox);
                Shifr();
        }
        ...
        private BitArray DoDirectMainCicl(BitArray bitArray)
        {
            ...
            var shifr = Transposition(concat, IP_Bi, false);
            return shifr;
        }
        ...
        {
            var shifr = RevTransposition(bitArray, IP_Bi, false);
            var leftAndRight = SplitInHalf(shifr);
            var left = leftAndRight[0];
            var right = leftAndRight[1];
            var oldRight = right;
            //var K = GetK();
